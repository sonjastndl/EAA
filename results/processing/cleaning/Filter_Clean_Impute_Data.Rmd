...
Env85 <-  Env85[rownames(Env85) %in% Env85_samples_cols,] ##Env85 <-  EnvNULL[rownames(EnvNULL) %in% Env85_samples_cols,] 
EnvCore <- Env85
...
#non_monomorphic <- sapply(EnvCore, function(col) length(unique(col)) > 1)
#EnvCore <- EnvCore[, non_monomorphic]
#
#
...
#col_monomorphs <- apply(EnvCore, 2, function(column) is_monomorphic(column, threshold = 0.7))
#EnvStats <- EnvCore[,!col_monomorphs]
#EnvDataStats <- as.data.frame(lapply(EnvStats, as.numeric))
#s <- t(summary(EnvDataStats))
#write.table(s, file="/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/Cli#mateData/Stats/SummaryEnv.csv")

```


```{r addMetadata, echo=FALSE}
metadata <- read.csv("/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/ClimateData/Stats/metadata_corrected.csv", header=TRUE)
...
# Apply the function to each column of EnvCore_imputed
#col_monomorphs <- apply(EnvCore_imputed, 2, function(column) is_monomorphic(column, threshold = 0.7))
...
for (start in seq(1, num_cols, by = 12)) {
  ...
  #pdf(file = sprintf("/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/ClimateData/Stats/ValidationAfterImputation/Validatio_after_Correction_and_Imputation_%d.pdf", start), width = 14, height = 11)
...
Coordinates$names <- rownames(Coordinates)
...
p <- ggplot() +
    ...
    geom_point(data = Coordinates, aes(x = Long, y = Lat), color = "#7aa444", size = 3) +
    coord_fixed(1.3, xlim = c(-25, 45), ylim = c(35, 70)) + ggtitle(paste("Samples remained after filtering (",nrow(Coordinates),"): Strategy A", sep=""))
    theme_minimal()
